...
impl Seed256 {
    pub fn from_hex(hex: &str) -> Result<Self, String> {
        if hex.len() != 64 { return Err("seed hex must be 64 chars".to_string()); }
    ...
    pub fn to_hex(&self) -> String {
        self.0.iter().map(|v| format!("{v:016x}")).collect::<Vec<String>>().join("")
    }
...
impl Xoshiro256PlusPlus {
    pub fn from_seed(seed: Seed256) -> Self { Self { state: seed.0 } }
    pub fn from_string(seed: &str) -> Self { Self::from_seed(Seed256::from_string(seed)) }
